...
### Aktifkan Firebase Storage (untuk fitur upload referensi)

Form order punya field opsional untuk upload file referensi (gambar/PDF).
File ini disimpan di Firebase Storage, bukan Firestore. Cara aktifkan:

1. Di Firebase Console, buka **Build → Storage**, klik **Get started**,
   pilih mode **production**, ikuti langkah setup-nya.
2. Setelah aktif, nama bucket-mu akan terlihat di bagian atas halaman
   Storage, formatnya seperti `nama-project.appspot.com` atau
   `nama-project.firebasestorage.app`.
3. Tambahkan sebagai environment variable `FIREBASE_STORAGE_BUCKET`.
4. Di tab **Rules** pada halaman Storage, kunci akses langsung dari
   browser (sama seperti Firestore), karena semua upload dilakukan lewat
   API route di server:
   ...
   File tetap bisa dibuka oleh kamu (admin) lewat link signed URL yang
   otomatis dibuat server saat ada order masuk dengan referensi, atau
...
3. Sebelum klik Deploy, buka bagian **Environment Variables** dan tambahkan
   empat variabel ini (nilainya dari file JSON service account di langkah 1,
   plus nama bucket Storage):
   - `FIREBASE_PROJECT_ID`
...
kebutuhan, deadline, status (`baru`), waktu order dibuat, dan jika
pelanggan upload referensi, ada juga `referenceUrl` (link untuk membuka
file-nya langsung) dan `referenceFileName`.
